...
TcpSocketImpl::TcpSocketImpl(Loop *loop) : data_(nullptr), loop_(nullptr)
{
    if (loop == nullptr)
    {
        return;
    ...
    data_ = new Data;
    if (0 != uv_tcp_init((uv_loop_t *)loop->GetHandle(), (uv_tcp_t *)data_))
    {
        delete data_;
        data_ = nullptr;
    }
...
bool TcpSocketImpl::Connect(const EndPoint &ep, OnConnectCb &&cb)
{
    if (data_ == nullptr)
    {
        return false;
...
bool TcpSocketImpl::Connect(const String &host, uint16_t port, OnConnectCb &&cb)
{
    return Connect(EndPoint(host, port), std::forward<OnConnectCb>(cb));
}

bool TcpSocketImpl::Bind(const EndPoint &ep)
{
    ...
    return 0 == uv_tcp_bind((uv_tcp_t *)data_, &ep.impl_->addr_, 0);
}

bool TcpSocketImpl::Listen(int backlog, TcpSocket::OnConnectionCb &&cb)
{
    if (data_ == nullptr)
    {
        return false;
...
bool TcpSocketImpl::Send(const void *data, uint32_t length, OnSendCb &&cb)
{
    if (data_ == nullptr)
    {
        return false;
...
bool TcpSocketImpl::StartReceive(OnDataCb &&cb)
{
    if (data_ == nullptr)
    {
        return false;
...
bool TcpSocketImpl::ShutDown(TcpSocket::OnShutDownCb &&cb)
{
    struct Req
    {
        uv_shutdown_t req{};
