...
public:
    explicit Impl(InputStream *is)
        : inputStream_(is), lz4Stream_(nullptr), lz4StreamDecode_(), cmpBuf_(), decBuf_(), decBufIndex_(0)
    {
    ...
    int32_t Read(void *buf, uint32_t count) override
    {
        ...
        while (count != 0)
        {
            ...
            {
                const int decBytes =
                    LZ4_decompress_safe_continue(&lz4StreamDecode_, (const char *)cmpBuf_.GetBufferHead(),
...
Lz4InputStream::Lz4InputStream(InputStream *stream) : impl_(new Impl(stream))
{
}
