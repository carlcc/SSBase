...
public:
    explicit InputStreamReader(InputStream *stream) : stream_(*stream)
    {
    ...
    uint32_t ReadUint32LE(bool *ok = nullptr) const
    {
        ...
        return (uint32_t(data[3]) << 24u) | (uint32_t(data[2]) << 16u) |
               ((uint32_t(data[1]) << 8u) | uint32_t(data[0]));
    }
    ...
    uint64_t ReadUint64LE(bool *ok = nullptr) const
    {
        ...
        return (uint64_t(data[7]) << 56u) | (uint64_t(data[6]) << 48u) | (uint64_t(data[5]) << 40u) |
               ...
               (uint64_t(data[1]) << 8u) | uint64_t(data[0]);
    }
